...
function DealResults({ response }: { response: unknown }) {
  ...
  const createdDealName = display(root?.createdDealName);

  return (
    <div className="space-y-4">
      {createdDealName !== "—" && (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
          <strong>Unique deal name created:</strong> {createdDealName}
        </div>
      )}
      {opportunity ? (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          <Metric label="Deal ID" value={display(opportunity.id)} />
          <Metric label="Name" value={display(opportunity.name)} />
          <Metric label="Amount" value={display(opportunity.amount)} />
          <Metric label="Close date" value={display(opportunity.closed_date)} />
          <Metric label="Account ID" value={display(opportunity.account_id)} />
          <Metric label="Stage" value={display(opportunity.opportunity_stage_id ?? opportunity.stage_id)} />
        </div>
      ) : (
        <EmptyState text="Apollo returned success but no deal object was found in the response." />
      )}
      <details className="rounded-2xl border border-slate-200 p-4">
        <summary className="cursor-pointer text-sm font-bold text-slate-700">Raw safe response</summary>
        <pre className="mt-3 max-h-[420px] overflow-auto rounded-xl bg-slate-950 p-4 text-xs leading-6 text-white">{JSON.stringify(response, null, 2)}</pre>
      </details>
    </div>
  );
}
  ...
  return <main className="min-h-screen bg-[#f7f9fc] p-4 text-slate-950 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1400px]">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[330px_1fr]"><aside className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><p className="px-2 pb-3 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Apollo API samples</p><div className="space-y-2">{operations.map((item) => { const itemBadge = badge(item.status); return <button key={item.id} type="button" onClick={() => chooseOperation(item.id)} className={`w-full rounded-2xl border p-4 text-left transition ${item.id === operation ? "border-blue-200 bg-blue-50" : "border-transparent hover:border-slate-200 hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-extrabold text-slate-900">{item.title}</p><p className="mt-1 text-xs font-medium text-blue-700">{item.subtitle}</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-black ${itemBadge.cls}`}>{itemBadge.text}</span></div></button>; })}</div></aside>
      <section className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><div className="border-b border-slate-100 pb-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-600">Selected API</p><h2 className="mt-2 text-2xl font-extrabold text-[#12315b]">{active.title}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{active.description}</p></div><span className={`w-fit rounded-full px-3 py-1.5 text-[10px] font-black uppercase ${activeBadge.cls}`}>{activeBadge.text}</span></div><div className="mt-4 rounded-xl bg-blue-50 p-4 text-xs leading-5 text-blue-900"><strong>Help:</strong> {active.help}</div></div>
        {active.status === "PAID_PLAN" ? <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">This operation is disabled on the current Apollo Free plan.</div> : <form onSubmit={run} className="mt-6">
          {operation === "contacts-search" && <div className="grid gap-5 md:grid-cols-2"><Field label="Contact keywords" value={form.keywords} onChange={(value) => setField("keywords", value)} help="Optional. Search saved contacts. Leave blank for the first page." /><Field label="Results per page" type="number" value={form.perPage} onChange={(value) => setField("perPage", value)} help="Use 10 while testing; V-Bridge caps the value at 100." /></div>}
          {operation === "accounts-search" && <div className="grid gap-5 md:grid-cols-2"><Field label="Company / account name" value={form.organizationName} onChange={(value) => setField("organizationName", value)} help="Optional. Search companies already saved as Apollo accounts." /><Field label="Results per page" type="number" value={form.perPage} onChange={(value) => setField("perPage", value)} help="Number of saved accounts to return." /></div>}
          {operation === "users" && <div className="grid gap-5 md:grid-cols-2"><Field label="Page number" type="number" value={form.page} onChange={(value) => setField("page", value)} help="Start with page 1." /><Field label="Results per page" type="number" value={form.perPage} onChange={(value) => setField("perPage", value)} help="Number of workspace users to return." /></div>}
          {operation === "deal-create" && <div className="grid gap-5 md:grid-cols-2"><Field label="Deal base name" required value={form.dealName} onChange={(value) => setField("dealName", value)} help="Enter a readable base name. V-Bridge makes the actual Apollo deal name unique automatically." placeholder="ads new deal" /><Field label="Amount" type="number" value={form.amount} onChange={(value) => setField("amount", value)} help="Optional numeric deal amount. 0 is allowed." /><Field label="Estimated close date" type="date" value={form.closedDate} onChange={(value) => setField("closedDate", value)} help="Optional. Leave blank if the deal does not yet have an estimated close date." /><Field label="Apollo account ID" value={form.accountId} onChange={(value) => setField("accountId", value)} help="Optional. If the deal belongs to a saved Apollo account, enter its account ID. Leave blank otherwise." /></div>}
          {operation === "deal-view" && <Field label="Deal ID" required value={form.dealId} onChange={(value) => setField("dealId", value)} help="Apollo opportunity ID. After successful Create Deal, V-Bridge automatically fills this field for you." placeholder="6a8bdffab8de17001c360e12" />}
          {noInputs && <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 text-sm leading-6 text-blue-950">No test input is required for this endpoint.</div>}
          <div className="mt-6 flex flex-wrap items-center gap-3"><button type="submit" disabled={loading} className={`rounded-xl px-5 py-3 text-sm font-extrabold text-white shadow-lg disabled:opacity-60 ${operation === "deal-create" ? "bg-amber-600 hover:bg-amber-700" : "bg-[#0b4ea2] hover:bg-[#083d80]"}`}>{loading ? "Calling Apollo…" : `Run ${active.title}`}</button><span className="text-xs text-slate-500">This creates a visible V-Bridge API request in Chrome Network. Apollo API key remains on the server.</span></div>
        </form>}
        {active.status !== "PAID_PLAN" && <div className="mt-7 border-t border-slate-100 pt-6"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Apollo response</p><p className="mt-1 text-sm font-bold text-slate-700">{httpStatus === null ? "Run the sample to see results." : httpStatus === 0 ? "Gateway error" : `HTTP ${httpStatus}`}</p></div>{httpStatus !== null && httpStatus >= 200 && httpStatus < 300 && <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-black text-emerald-700">SUCCESS</span>}</div><div className="mt-4">{response === null ? <EmptyState text="No response yet." /> : operation === "contacts-search" ? <ContactResults response={response} /> : operation === "deal-create" || operation === "deal-view" ? <DealResults response={response} /> : <GenericResults response={response} />}</div></div>}
      </section></div>
  </div></main>;
}
